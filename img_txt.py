...
def compute_cell_positions(step_x, step_y, grayscale_puzzle, model, unsolved_board):
    ...
    for y in range(9):
        ...
        for x in range(9):
            ...
            if digit is not None:
                recognize_digit(digit, x, y, model, unsolved_board)
...
def recognize_digit(digit, x, y, model, unsolved_board):
...
# This function displays the solutions of the sudoku puzzle on the color puzzle image and returns the RGB image.
def display_solutions_on_image(cell_positions, color_puzzle, unsolved_board, solved_board):
    ...
    for (cell_row, unsolved_board_row, solved_board_row) in zip(cell_positions, unsolved_board, solved_board):
        ...
        for (box, unsolved_digit, solved_digit) in zip(cell_row, unsolved_board_row, solved_board_row):
            if unsolved_digit == 0:
                ...
                # Draw the digit on the sudoku puzzle image (use cyan for visibility)
                cv2.putText(
                    color_puzzle, str(solved_digit), (text_x, text_y),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 0), 2)  # Changed to cyan (BGR: 255,255,0)
    # Convert BGR image to RGB for displaying
    color_puzzle_rgb = cv2.cvtColor(color_puzzle, cv2.COLOR_BGR2RGB)
    return color_puzzle_rgb

# Main processing function (refactored for Streamlit, no file I/O)
def process_image(input_image, model):
    ...
    cell_positions = compute_cell_positions(step_x, step_y, grayscale_puzzle, model, unsolved_board)
    unsolved_board = unsolved_board.tolist()
    return unsolved_board, cell_positions, color_puzzle
